"""Model backends: the hosted API, or a model running locally.

`review()` takes a client by injection, so a backend only has to expose
...
A local reviewer is not equivalent to a frontier one and this module does not
pretend otherwise. What it buys is the ability to develop, debug and regression
test a reviewer with no API key and no per-call cost. Which model produced a
review is recorded on every result, so a local score is never mistaken for a
hosted one.
"""
...
DEFAULT_LOCAL_MODEL = os.environ.get("PR_REVIEWER_LOCAL_MODEL", "qwen3:8b")
...
class OllamaClient:
    """Minimal adapter presenting the Messages surface the reviewer expects.

    Ollama speaks its own chat API rather than the Messages protocol, so rather
    than pulling in a translation proxy this maps the three fields the reviewer
    ...
    hoping. The parser still tolerates prose around the object, because not every local
    model honours the constraint.
    """
...
def build_client(backend: str = "auto", model: str | None = None):
    """Pick a backend. "auto" prefers the hosted API, falling back to local."""
    if backend == "local":
